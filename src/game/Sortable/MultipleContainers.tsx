...
import { DroppableContainer } from "@/game/Sortable/DroppableContainer";
import { Items } from "@/game/Sortable/types";
...
export function MultipleContainers({
	...
	const [items, setItems] = useState<Items>(
		() => initialItems ?? dummyData(itemCount)
	);
	...
	const sensors = getSensors(coordinateGetter);

	const getIndex = getIndexHelper(items);
	const onDragCancel = OnDragCancel(
		clonedItems,
		setItems,
		setActiveId,
		setClonedItems
...
function dummyData(itemCount: number): Items {
	return {
...
function getIndexHelper(items: Items) {
...
function getSensors(coordinateGetter: KeyboardCoordinateGetter) {
	return useSensors(
...
function getCollisionDetectionStrategy(
	activeId: UniqueIdentifier | null,
	items: Items,
...
function findContainer(id: UniqueIdentifier, items: any) {
	if (id in items) {
	  return id;
	}
